...
static void wrapper_add_cc(int cc_number, const char* display_name, const char* persist_name, int default_value) {
...
static double wrapper_get_sample_rate(void) {
  return jack_get_sample_rate(jack_client);
}

#define wrapper_port_get_buffer jack_port_get_buffer

static void wrapper_add_audio_input(const char* name, float** buf) {
...
static void wrapper_add_audio_output(const char* name, float** buf) {
  CHECK(jack_num_ports < MAX_NUM_PORTS, "too many ports");
  int i = jack_num_ports++;
